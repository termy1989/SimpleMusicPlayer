...
class MusicListPresenter(initView: MusicListView, initService: DataService) {
    ...
    private var currentAudioTrackTitle = ""                                                     // наименование текущего трека
    private var currentAudioTrackStatus = Defines.TRACK_IS_STOPPED                              // статус текущего трека
    private var mView = initView                                                                // интрефейс
    private var mService = initService                                                          // сервис работы с хранилищем
    private var mCurrentList = ""                                                               // текущий плейлист
    private var mCachedList: ArrayList<AudioTrack>? = null                                      // сохраненное содержимое плейлиста
    ...
    fun refresh(title: String?, status: Int) {
        ...
        if (mCachedList != null) {
            ...
            mView.showTracks(mCachedList!!, mService.restoreSortOrder(mView as Context))
            mCachedList = null
        }
        ...
        // поиск и вывод треков
            ...
            if (cursor != null) {
                ...
                else mView.showTracks(songsList, mService.restoreSortOrder(mView as Context))
                cursor.close()
    ...
    fun selectItem(item: AudioTrack, list: ArrayList<AudioTrack>) {
        ...
        mView.showTracks(newArray, mService.restoreSortOrder(mView as Context))
    }
    ...
    // сортировка по наименованию трека
    fun selectSortOrder(sort: Int) {
        mService.saveSortOrder((mView as Context), sort)
    }
}