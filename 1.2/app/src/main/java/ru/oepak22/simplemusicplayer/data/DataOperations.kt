...
import ru.oepak22.simplemusicplayer.content.*
...
class DataOperations @Inject constructor() : DataService {
    ...
    // сохранение порядка сортировки треков
    override fun saveSortOrder(context: Context, order: Int) {
        val settings = context.getSharedPreferences("Settings", Context.MODE_PRIVATE)
        val editor = settings.edit()
    ...
    // восстановление порядка сортировки треков
    override fun restoreSortOrder(context: Context): Int {
        val settings = context.getSharedPreferences("Settings", Context.MODE_PRIVATE)
        return if (settings.contains("Sort_Order"))
            settings.getInt("Sort_Order", Defines.SORT_BY_TITLE)
        else Defines.SORT_BY_TITLE
    }
